...
app.post('/users/show_room', (req, res) => {
    conection.query(`select room_id, name, people from room`, function (error, results, fields) {
        try {
            res.status(201).json({
                message: "Successful",
                data: results
...
app.post('/users/change_username', (req, res) => {
    ...
    conection.query(`update users set username = '${new_username}' where user_id = '${user_id}'`, function (error, results, fields) {
        try {
            if (error) throw error;
            res.status(201).json({
                message: "Successful",
...
app.post('/users/getAcard', async (req, res) => {
    ...
    try {
        ...
        async function main() {
            await sleep(1000);
            var sql = `SELECT card.name, user_id, room_user.room_id, result_point, chair, card.card_id FROM room_user_card LEFT JOIN card ON room_user_card.card_id = card.card_id LEFT JOIN room_user ON room_user_card.room_user_id = room_user.room_user_id WHERE room_user.room_id = '${room_id}' and room_user.user_id = '${user_id}' LIMIT 1 OFFSET 2`;
            const [result_table] = await con.query(`select chair from room_user where user_id = '${user_id}' and room_id = '${room_id}'`);
