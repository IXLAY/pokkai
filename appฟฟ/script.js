  ...
  axios.post('http://127.0.0.1:3000/users/choose_chair', data)
    .then(response => {
      if (response.data.message === 'Successful') {
        ...
        document.getElementById('money_player' + i).innerHTML = '0 à¸¿'
      ...
      if (response.data.message === 'Successful') {
        ...
        for (let i = 0; i < players.length; i++) {
          ...
          if (player.chair != 0) {
            ...
            document.getElementById('money_player' + player.chair).innerHTML = player.bet + ' à¸¿'
    ...
    .then(response => {
      if (response.data.message === 'Successful') {
        ...
        for (let n = 1; n < 6; n++) {
          ...
          var check =  document.getElementById('name_player' + n).textContent
          if (check !== 'ชื่อผู้ใช้' && document.getElementById('money_player' + n).innerHTML != '0 à¸¿')  {
  ...
  axios.post('http://127.0.0.1:3000/users/getAcard', data)
    .then(response => {
      ...
      if (response.data.message === 'Successful') {
        ...
        playerCards[2].src = `/image/card/${returnValue.card_id}.svg`;
  ...
  .then(response => {
    if (response.data.message === 'Successful') {
      ...
      for (let n = 1; n < 6; n++) {
        ...
        betContainer.classList.add('active');
        betbutton.classList.add('active');
        document.getElementById('data-player' + data.chair).classList.remove('active');
        document.getElementById('pic_player' + data.chair).classList.remove('active');
